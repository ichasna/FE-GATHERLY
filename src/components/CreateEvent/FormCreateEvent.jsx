...
function FormCreateEvent(props) {
  ...
  function memberChangeHandler(id, event) {
    ...
    const newArr = newMember.map((i) => {
      return i.eventMember;
    });
  ...
  function submitHandler(event) {
    ...
    props.onSaveMembersData(membersData);

    setEnteredEvent("");
    setEnteredDescription("");
    setInputFields([{ id: uuidv4(), eventMember: "" }]);
        ...
        <div className="flex flex-col w-full">
          ...
          {inputFields.map((inputField) => (
            <div key={inputField.id} className="flex flex-col w-full">
              <input
                name="eventMember"
                type="text"
                value={inputField.eventMember}
                placeholder="Type your member team username here"
          ...
          <div className="flex flex-row justify-center">
            <button
              ...
              className="border border-teal-500 mx-2 p-2 rounded-lg hover:scale-105 duration-300"
            >
              {" "}
              Remove New Member
            </button>
            ...
            <button
              ...
              className="border border-teal-500 mx-2 p-2 bg-rose-500 hover:scale-105 duration-300 rounded-lg "
            >
              {" "}
              Add New Member
            </button>
          ...
          <button className="w-auto m-4 mt-8 mx-36 px-3 py-1 sm:px-5 text-md sm:text-lg text-white bg-gradient-to-b from-[#C44054] to-[#DB77A5] rounded-lg hover:from-pink-500 hover:to-yellow-500 hover:scale-105 duration-300">
            Create Event
          </button>
        </div>
      </div>
